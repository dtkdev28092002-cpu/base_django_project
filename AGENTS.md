...
- `user/views.py` - API views (auth, user detail, tags with pagination, health check), custom `PaginationData` class
...
Custom pagination is defined in `PaginationData` class in `user/views.py`. To reuse for other list endpoints:
